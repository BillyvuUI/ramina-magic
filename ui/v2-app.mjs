...
let anchor = Date.now(), elapsed = performance.now(), ui, game, connected = false, evaluatedWeek = null;
...
async function connectSession() {
    ...
    if (document.getElementById('gameHome')) {
      ...
      game = createGameUI({ document, store, clock, rewards, schedule: ui, ready:completion.ready, onError: report,
        async loadHistory(isParent, cursor) {
          if (isParent && !parentUnlocked) throw new Error('Неверный PIN');
  ...
  status.textContent = session.mode === 'maintenance' ? 'Идёт безопасное обновление приложения' : `Синхронизация включена · Алматы · ${session.today}`;
...
document.getElementById('parentOpen').onclick = () => {
...
document.getElementById('parentForm').onsubmit = async event => {
  ...
  try {
    ...
    status.textContent = 'Родительский режим';
    if (game) game.openParent(); else ui.openParent();
